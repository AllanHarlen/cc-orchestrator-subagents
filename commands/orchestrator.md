...
Antes/depois de cada fase, persista `phase --status RUNNING|DONE|FAILED|BLOCKED|CANCELLED|UNKNOWN` com evidencia. Antes de lancar um subagente, crie a worktree elegivel, adquira lease e persista a task como `RUNNING` com executor, modelo, attempt e `sessionId`/`conversationId`. Assim que a ultima task da wave estiver `RUNNING`, inicie `orchestration-lifecycle.mjs watch` em segundo plano — obrigatorio, com ou sem `--adapter-config`. Sem status autoritativo, o watcher preserva o ultimo `RUNNING` como nao verificado; somente o sweeper pode marcar `STALLED` depois do grace period. `updateCompletionGate --gate monitoring --status DONE` recusa fechar sem ao menos um tick/sweep ter rodado. O retorno externo redigido deve ser persistido antes de `DONE`/`FAILED`/`BLOCKED` e antes de anunciar o resultado. Use apenas estados canonicos e preserve quota/auth/timeout em `reasonCode`.
