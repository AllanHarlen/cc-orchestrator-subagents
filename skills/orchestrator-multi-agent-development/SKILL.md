...
11a. Para plano PRD com `requirementIds`, antes de fechar a Fase 10 grave `review/requirements-evidence.json` a partir de `assets/requirements-evidence.template.json`: cada criterio de aceite precisa de status `PASS`, evidencia concreta e nenhum finding aberto. Sem isso o audit recomenda `PARTIAL` e o gate `requirementsCoverage` nao pode fechar `DONE`. `requirements.json` do Pensador carrega tres listas de id — `requirements` (RF, ligado a `CA-XX` via `acceptanceCriteria`), `nonFunctionalRequirements` (RNF, secao 7 do PRD) e `architecturePatterns` (ARC sinteticos, secao 15 "Padroes de Arquitetura & Design") — e o gate de cobertura (Fase 2/7) exige task para os tres, nao so RF. RNF/ARC nao tem `CA-XX` vinculado no PRD: registre em `requirements-evidence.json` uma entrada `requirementId: "RNF-01"` (ou `ARC-01`) com um criterio proprio em `acceptanceCriteria` (ex.: `{"id": "RNF-01-VERIFICADO", "status": "PASS", "evidence": [...]}` — ver o exemplo no template), com evidencia concreta (teste de carga para um RNF de performance, o Repository de fato presente no codigo para um ARC de "Repository + UnitOfWork") — nunca marque PASS so porque uma task cita o id em `requirementIds`. O gate `requirementsCoverage` **le o conteudo** do arquivo ao fechar (`REQUIREMENTS_EVIDENCE_BLOCKED`): toda id reivindicada por task e, com o snapshot `plan/requirements-index.json` (gravado por `validate-requirements-coverage.mjs --dir`), toda id do indice, todo `CA-XX` vinculado a cada `RF`, e evidencia `kind: "test"` para `RNF` de seguranca/privacidade/isolamento/conformidade — uma run real fechou esse gate so com a existencia do arquivo enquanto o review reprovava 7 CAs. Uma run real (OficinaAI, 2026-09) fechou com todo RF coberto enquanto RNF de performance/isolamento por tenant e o padrao Repository + UnitOfWork do PRD nao tinham nenhuma task nem evidencia — nada aqui olhava para essas duas listas antes desta versao.
...
13. Antes de delegar, persista o prompt em `run/prompts/<taskId>.md` e meca-o com `scripts/check-prompt-budget.mjs --agent agy|codex --file <path>` (nunca conte manualmente). Desde o bridge cc-antigravity-plugin 4.4.0 (stream do prompt final via stdin, sem descarte de contexto), a checagem e indicativa para os dois agentes — nunca bloqueia o dispatch — mas um `ok: false` continua sinal de escopo mal recortado: considere dividir a task em subtasks por entregaveis. Para pacote de design (Open Design), use `--design-system <materializeInto>` no lugar de colar `tokens.css`/`components.html`/`DESIGN.md` em `--priority-files`: o bridge entrega o pacote inteiro fora desse orcamento (ver Secao 2a de `references/subagent-prompts.md`).
...
15. **Design system (Open Design) e contrato visual, nao decoracao.** Quando a especificacao tiver design system — `design-system.md` (modo PRD) ou `design.md` + `specs/ui-design-system/spec.md` (modo Spec OpenSpec) — o orquestrador primeiro **materializa** o pacote `resolved/` do Pensador (`design-system-files`, em `.pensador/<slug>-vN/design-systems/<id>/resolved/`) para o alvo real via `materializeInto`, depois de conferir mecanicamente o `contractSha256` e o `design-audit.json` (nunca o `validation.status` do handoff) (ex.: `packages/ui/design-systems/<id>/` — `tokens.css`, `components.html`, `preview/`; ver Fase 4.0 e `references/handoff-contract.md` secao 6). Em seguida **passa os caminhos materializados no prompt de toda task front-end** (com o `design-brief.json` e os temas) e exige que o AGY **consuma `design-contract.json`/`tokens.css` (sem inventar tokens)** e bata os componentes com `components.html`, na ordem normativa `design-contract.json`/`tokens.css` > `components.html` > prosa do `DESIGN.md`; o wave gate (`run-wave-gate.mjs --tokens-css`) reprova hex literal, px de espacamento/raio e `style={{}}` inline nos arquivos alterados. O `preview/` gerado do design system e a spec visual: o AGY replica layout e hierarquia sem inferir telas do zero, e o review da Fase 9 valida a fidelidade contra ele. Ativos de marca (`brand-assets` em `assets/` e `manifest.json`) sao consumidos sem inventar nem gerar imagens. Na Fase 9, o review aplica o **gate de design**: `tokens.css` consumido via `var(--*)`, accent contido (≤ 2x/pagina), fidelidade visual contra `prototypes/`, telas-chave conferidas contra o diretorio `preview/` gerado do contrato (`index.html`, `colors.html`, `typography.html`, `spacing.html`, `components.html`, `app.html`, nos temas claro e escuro), anti-padroes da secao 9 ausentes; violacao de requisito explicito e BLOQUEANTE.
...
`--effort <low|medium|high>` (alias legado: `--agy-effort`) e override publico apenas de implementacao; o review continua em effort `high`. `--timeout <duracao>` (alias legado: `--agy-timeout`) vale para todas as delegacoes AGY, inclusive review. Quando o handoff carrega o role `design-prototype` (Open Design, Fase 4), o prompt de toda task front-end recebe o caminho do prototipo junto com `design-system-files`/`tokens.css`, e a task delega a reproducao das telas na stack de front-end do projeto ao AGY — nao ha script de conversao mecanica. Nao exponha diretamente `--mode`, `--format`, `--agent`, `--json-schema`, `--continue` ou `--conversation`: o orquestrador escolhe esses controles pelo papel e pelo estado persistido. `--agent` significa agente customizado do AGY, nao subagente Claude.
...
4. Validar roteamento/wire format, materializar arquivos de design (Open Design) via `materializeInto` (e, quando houver `design-prototype`, incluir seu caminho no prompt das tasks front-end para reproducao na stack do projeto) e criar contratos obrigatorios
...
- [ ] completion gates aplicaveis (`visualMaterialization`, `contractsInspected`, `infraSmokeTest`, `monitoring`, `apiContractValidation`, `backendReview`, `frontendReview`, `visualAudit`, `browserE2E`, `reports`, `handoff`, `delivery`, `learning`, `requirementsCoverage`) estao `DONE` com evidence IDs; gates N/A possuem motivo e nunca substituem gate obrigatorio
...
- [ ] Fase 4 rodou `validate-contract-coverage.mjs` (gate `contractCoverage`, nao rastreado em `orchestration-state.mjs` — mesmo padrao exit-code de `validate-requirements-coverage.mjs`) antes de qualquer dispatch front-end; `applicable: false` (sem `ui-data-map`/contrato) registrado em `report/workflow-log.md`, nunca tratado como PASS silencioso
...
- [ ] quando o upstream trouxe `ui-data-map` (ou foi montado na Fase 2, modo independente): `validate-contract-coverage.mjs` executado na Fase 4 ANTES de qualquer dispatch front-end — toda `reads[]`/`writes[]` de tela casa com uma operacao real do contrato; `CONTRACT_COVERAGE_GAP` vira task de back-end antes do front-end ser despachado, nunca contornado com dado local. Prompts front-end declaram `ui-data-map.json` como unica fonte de dados (proibido `localStorage`/`sessionStorage`/estado do cliente para entidade de dominio); retorno de toda task front-end traz "Fonte de dados por tela"
...
- [ ] `design-contract.json`, `tokens.css`, `DESIGN.md` e `assets/manifest.json` foram carregados como priority-files; apenas o pacote resolved autoritativo e seus assets foram materializados, com `seedBindings`, sem pergunta ou geracao de imagem no Orquestrador
- [ ] cada task front-end registra `visualImageryPolicy` (`required`/`not-applicable` — a politica vem da superficie da task, nao de palavras soltas como "hero"/"banner"); em modo conjunto preserva `project-baseline.json.visualImageryPlan`, em modo independente usa `visual-imagery-plan.mjs`; toda geracao tem `AGY_IMAGE_RESULT` e vinculo real
