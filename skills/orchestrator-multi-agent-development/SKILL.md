...
- [ ] cada task front-end registra `visualImageryPolicy`; em modo conjunto preserva `project-baseline.json.visualImageryPlan`, e em modo independente gera via AGY quando `required` ou documenta a decisao quando `recommended`; toda geracao tem `AGY_IMAGE_RESULT` e vinculo real
