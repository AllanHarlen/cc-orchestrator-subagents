...
13. Antes de delegar, persista o prompt em `run/prompts/<taskId>.md` e meca-o com `scripts/check-prompt-budget.mjs --agent agy|codex --file <path>` (nunca conte manualmente). Para AGY o limite de 24.000 chars e duro — divida a task em subtasks por entregaveis antes de delegar. Para Codex (`--prompt-file`, sem limite de argv) a mesma checagem e so indicativa; nunca bloqueia o dispatch.
...
- [ ] prompts persistidos em `run/prompts/` e verificados com `check-prompt-budget.mjs` antes da delegacao; prompts AGY ficam abaixo de 24.000 caracteres e sao divididos quando excedem o limite
