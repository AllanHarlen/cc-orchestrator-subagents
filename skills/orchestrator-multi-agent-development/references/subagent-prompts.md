...
- importe `components.css` do pacote materializado no stylesheet global, logo depois de `tokens.css`/`tailwind-v4.css` — e a unica folha de componentes do pacote. NUNCA copie `preview/preview.css` nem as classes de andaime de `components.html` (`.page`, `.scope`, `.grid`, `.state`): `.grid` do preview e `display:flex` e quebra o utilitario `grid` do Tailwind. Pacote antigo sem `components.css`: extraia so as regras de componente para um arquivo proprio e registre isso no relatorio. Componentes que o `USAGE.md` lista como sem regra dedicada sao construidos a partir dos tokens, cobrindo todos os estados de `components.manifest.json`. Numa run real, `Button`/`Card`/`Input`/`Badge`/`Dialog` chegaram ao bundle final sem nenhum estilo porque so `tokens.css` foi importado — typecheck/lint/build passaram;
...
- as classes de componente do pacote (`.btn`, `.input`, `.card`, `.badge`, `.alert`, `.modal`) usadas pelo codigo existem no **CSS compilado** do build (ex.: `grep -c '\.btn\b' <dir de saida do build>/**/*.css`), nao so no fonte — typecheck, lint, build e o lint de tokens nao detectam classe sem regra;
