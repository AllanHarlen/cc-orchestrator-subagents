...
- Em modo conjunto, ingira os artefatos do Pensador na ordem do handoff contract (secao 7):
  - **Modo PRD:** `prd` → `userhistory` → `architecture` → `api-contract` → `communication-contract` → `ui-data-map` → `seed-plan` → `surface-benchmark` → `design-system`/`design-system-files` → `brand-assets`.
...
- **Brand Assets:** quando houver `brand-assets` (`assets/`), use-os como pacote de mídia fechado do Pensador nos prompts do AGY (Fase 5) e gate de fidelidade visual (Fase 9). (Handoffs que declaram o papel removido `ui-prototype` são bloqueados com `LEGACY_UI_PROTOTYPE_ROLE`; regere-os no Pensador >= 2.32.)
...
6. Os gates de design `undefinedTokens`/`hardcodedDesignValues`/`previewDivergence` **nao aceitam autodeclaracao**: `false` so vale com arquivos de evidencia mecanica em `evidence.designEvidence` (caminhos relativos ao `ui-evidence.json`): `tokenLint` = saida `--json` do `run-wave-gate.mjs` com `checks.designTokens` `PASS`, habilitado e sem violacoes; `previewDiff` = saida agregada do probe de runtime/comparacao com `preview/` (`status: "PASS"`, sem finding bloqueante). Sem o arquivo: `DESIGN_EVIDENCE_MISSING`; arquivo reprovado: `DESIGN_EVIDENCE_FAILED`. Marque `evidence.designSystem` quando houver design system.
7. Grave `review/ui-evidence.json`, rode `validate-ui-evidence.mjs --evidence <arquivo> [--ui-data-map <ui-data-map.json>]` — com `--ui-data-map`, o validador tambem cruza cada tela declarada contra `evidence.routes[]` (`SCREEN_COVERAGE_INCOMPLETE` se faltar) e exige `persistenceProof` em toda tela de lista (`PERSISTENCE_PROOF_MISSING`) — e somente entao feche `gate --gate visualAudit --status DONE --evidence file:review/ui-evidence.json`.
